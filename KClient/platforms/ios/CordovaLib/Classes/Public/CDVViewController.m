...
- (NSURL*)appUrl
{
    ...
    if ([self.startPage rangeOfString:@"://"].location != NSNotFound) {
        ...
        NSString* wwwPath = [NSString stringWithFormat:@"%@%@",self.documentPath,@"/www"];
        
        if ([[NSFileManager defaultManager] fileExistsAtPath:wwwPath])
        {
            //沙河里有WWW的情况下：
            //NSLog(@"沙盒里存在WWW文件，这需要请求版本号，然后判断目前版本是不是最新，是就直接从WWW启动，否这就调用分线程下载方法去下载：%@",wwwPath);
            NSString *documentIndexPath = [NSString stringWithFormat:@"%@%@",wwwPath,@"/app/index.html"];
            NSString* startFilePath = @"";
            if ([[NSFileManager defaultManager] fileExistsAtPath:documentIndexPath]) {
                //NSLog(@"沙盒里存在WWW下的/app/index.html文件：%@",documentIndexPath);
                startFilePath = documentIndexPath;
                appURL = [NSURL fileURLWithPath:startFilePath];
            }else{
                //index.html 不存在的时候就从工程里自带的index启动
                //NSLog(@"index.html 不存在的时候就从工程里自带的index启动：%@",self.startPage);
